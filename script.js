...
let newroles = []
let role = roles == 'true' ? 'admin' : 'user'
...
for (let i = 0; i < nums.length; i++) {
     if(0 <= nums[i]){
     newblock.push(nums[i])
}     
}
for (let i = 0; i < numbers.length ; i++) {
     if(0 <= numbers[i]){
     newblock.push(numbers[i])
}
}     
...
function valueandquantity(val, quan){
let result = [];
...
valueandquantity('a',5);
valueandquantity('f', 7);
